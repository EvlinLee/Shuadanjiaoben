...
def addcookies():
    ...
    lekeopener2 = request.build_opener(request.HTTPCookieProcessor(lekecj), request.HTTPHandler)
...
def login():
    ...
    login = parse.urlencode(login2).encode(encoding='utf-8')
    print(login)
    heard_dict = {
    ...
    res_login = lekeopener2.open(req_login)
...
def shuadan():
    ...
    while True:
        ...
        request.install_opener(lekeopener2)
        res = lekeopener2.open(req)
        res = res.read()
        ...
        # print(type(code))
        print('***********************第 ', count, ' 次请求刷单任务***********************\n', '乐客联盟15968772003：code =', code, ' msgs =',
              msgs)
        print()
        if code == 1:
            # 接单成功停止任务
